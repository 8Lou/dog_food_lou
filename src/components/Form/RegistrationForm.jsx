//ВАРИАНТ НАПИСАНИЯ ФОРМЫ ПРИ ПОМОЩИ БИБЛИОТЕКИ react-hook-form
...
export const RegistrationForm = ({ isRequired = true }) => {
  ...
  const [tags, setTags] = useState(""); //изначально пустая строка
      ...
      <form className=" form-example" onSubmit={handleSubmit(sendData)}>
        <div>
          <input
            ...
            type="text"
            //синтаксис библиотеки реакт-хук-форм; не перезагр-ет страницу; под капотом сама достает вэлью
            {...register("name", nameRegister)}
            placeholder="name"
          />
          {errors?.name && <span> {errors?.name.message}</span>}{" "}
          {/* не брать errors если он: null / undefined */}
        </div>
        <div>
          <input
            className="form__input"
